...
bool check_format(int type,String value){
    ...
    if(type == 0){
      ...
      final localisation = RegExp(r'^([A-Z]|[0-9]){4}L[0-9]{6}$');
...
class _MyAppState extends State<MyApp> {
 ...
 Future<void> _updateConnectionStatus(ConnectivityResult result) async {
  ...
  if(check_format(0, _scanBarcode) == false){
    ...
    Localisation loc =  Localisation(_scanBarcode,  DateTime.now().toIso8601String(), 0, user.matricule);
     ...
     loc.exists().then((exist) async{
        if(exist == true){
          ...
          Localisation localisation = await Localisation.get_localisation(_scanBarcode);
  ...
  Future<void> poursuivre_operation(BuildContext context)async{
    ...
    List<Localisation> locs = await Localisation.show_localisations();

    
    if(locs.length >= 1 ){
         Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) =>  Detail_Operation(localisation: locs.last),
                    ),
                  );
    }else{
          ...
          SnackBar(
            content:
             Row(
               ...
               children: [
                 Icon(Icons.info,color: Colors.white,size: 25),
                 Text("Vous n'avez aucune opération précédente",
            style: TextStyle(fontSize: 17.0),
            ),
               ],
             ),
                ...
                Padding(
                  ...
                  child: Builder(builder: ((context) {
                    switch( _connectionStatus){
                      case  ConnectivityResult.mobile:return 
                      IconButton(
                        onPressed: (){
                          showDialog(  
                            context: context,  
                      ...
                      default: return Icon(Icons.wifi_off_sharp);
                          ...
                          Container(
                            ...
                            child:TextButton(
                                ...
                                onPressed: ()async{
                                  ...
                                  await db.execute("DELETE FROM Localisation;");
                                  ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content:
                                    Row(
